...
export function BulkImportSecretsModal({
  ...
  const [detectedFormat, setDetectedFormat] = useState<BulkImportFormat | null>(null)
    ...
    >
      {step === 'paste' && (
        <PasteStep
          ...
          parsedCount={
            detectedFormat ? parseBulkImport(pasted).ok ?
              (parseBulkImport(pasted) as { secrets: ParsedSecret[] }).secrets.length
              : 0 : 0
          }
          onCancel={onClose}
          onNext={goToReview}
...
function PasteStep({
  ...
  const { t } = useTranslation()
  const canProceed = detectedFormat !== null && parsedCount > 0 && existingLoaded
...
function ReviewStep({
  ...
  async function getWalletKey(): Promise<Uint8Array> {
    ...
    if (!rsaPrivateKey) throw new Error('RSA private key not available — unlock first')
  ...
  const importMut = useMutation({
    ...
    onSuccess: (resp) => {
      ...
      notifications.show({
        ...
        message: t('wallets.bulkImport.doneMessage', { created, updated, skipped, failed }),
        autoClose: 6000,
      })
      onImported()
    },
        ...
        <Table>
          ...
          <Table.Tbody>
            {rows.map((row, i) => (
              <Table.Tr
                key={i}
                style={row.exists ? { background: 'rgba(255,165,0,0.08)' } : undefined}
              >
                <Table.Td>
                  <Checkbox
                    checked={row.include}
                ...
                <Table.Td>
                  <Text size="xs" ff="monospace" c="dimmed" lineClamp={1} title={row.value}>
