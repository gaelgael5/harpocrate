...
async function getAuthHeaders(): Promise<Record<string, string>> {
  if (!_tokenProvider) {
    console.warn(
      "[Harpocrate] tokenProvider not configured — no Authorization header",
    );
    return {};
  }
  ...
  if (!token) {
    console.warn(
      "[Harpocrate] tokenProvider returned null — no Authorization header",
    );
    return {};
  }
