...
def test_normalize_prefix() -> None:
    from app.services.remote_backup_providers.s3_compatible import S3CompatibleProvider
    assert S3CompatibleProvider._normalize_prefix("") == ""
    assert S3CompatibleProvider._normalize_prefix("/foo/") == "foo/"
    assert S3CompatibleProvider._normalize_prefix("foo") == "foo/"
    assert S3CompatibleProvider._normalize_prefix("/foo/bar") == "foo/bar/"
...
async def test_test_connection_calls_head_bucket() -> None:
    ...
    with patch.object(p, "_make_client", return_value=fake_client):
        await p.test_connection("any-prefix/")
...
async def test_upload_stream_buffers_and_uploads() -> None:
    ...
    with patch.object(p, "_make_client", return_value=fake_client):
        total = await p.upload_stream("pf/", "backup.tar.age", source())
...
@pytest.mark.asyncio
async def test_upload_stream_with_empty_prefix_uses_bare_filename() -> None:
    from app.services.remote_backup_providers.s3_compatible import S3CompatibleProvider
    ...
    with patch.object(p, "_make_client", return_value=fake_client):
        await p.upload_stream("", "backup.tar.age", source())

    args = fake_client.upload_fileobj.call_args.args
    assert args[2] == "backup.tar.age"
...
async def test_upload_stream_rejects_path_separators() -> None:
    ...
    with pytest.raises(ValueError, match="path separators"):
        await p.upload_stream("pf/", "../etc/passwd", empty())
