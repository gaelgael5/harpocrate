...
export function ApiKeysPage() {
  ...
  async function handleCreate() {
    ...
    if (permissions === 0) {
      notifications.show({ color: 'red', message: t('apiKeys.permissionsRequired') })
      return
    }
    if (!rsaPublicKey) {
  ...
  async function handleRevoke(keyId: string) {
    ...
    try {
      ...
      notifications.show({ color: 'green', message: t('apiKeys.revokeSuccess') })
    } catch (err) {
      const msg = err instanceof ApiError ? err.message : String(err)
      notifications.show({ color: 'red', title: t('common.error'), message: msg })
    }
  ...
  return (
    <Stack>
      ...
      <Group justify="space-between">
        ...
        <Group>
          <Button variant="outline" onClick={() => navigate(`/wallets/${walletId ?? ''}`)}>
            {t('common.back')}
          </Button>
