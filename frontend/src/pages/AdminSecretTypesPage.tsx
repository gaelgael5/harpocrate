...
import { fetchSecretTypes, deleteSecretType } from "@/lib/adminApi";
...
export function AdminSecretTypesPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<SecretTypeListItem | null>(
    null,
  );
    ...
    <Stack>
          ...
          <Table.Tbody>
            {data?.types.map((st) => (
              <Table.Tr
                ...
                style={{
                  cursor: "pointer",
                  opacity: st.deprecated_at ? 0.6 : 1,
                }}
              ...
              >
                ...
                <Table.Td>
                  {st.current_version ? (
                    <Badge color="brand" size="sm">
                      {t("secret_types.currentVersion", {
                        version: st.current_version.version,
                      })}
                    </Badge>
                  ) : (
                ...
                <Table.Td>
                  <Text size="xs" c="dimmed">
                    {t("secret_types.used", {
                      count: st.used_by_secrets_count,
                    })}
                  </Text>
                </Table.Td>
                <Table.Td>
      ...
      >
        <Stack>
          ...
          {deleteTarget && (
            <Text fw={700}>
              {deleteTarget.type}/{deleteTarget.sous_type}
            </Text>
          )}
          <Group justify="flex-end">
            ...
            <Button
              ...
              onClick={() =>
                deleteTarget && deleteMut.mutate(deleteTarget.type_uuid)
              }
            >
              {t("common.delete")}
            </Button>
