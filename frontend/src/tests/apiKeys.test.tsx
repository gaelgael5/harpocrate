      ...
      <MantineProvider>
        ...
        <QueryClientProvider client={makeQueryClient()}>
          <MemoryRouter initialEntries={['/wallets/wallet-1/api-keys']}>
            <Routes>
              <Route path="/wallets/:walletId/api-keys" element={<>{children}</>} />
            </Routes>
          </MemoryRouter>
        </QueryClientProvider>
      </MantineProvider>
...
function setupMocks(keys: ApiKeyItem[] = []) {
  ...
  mockUseCreateApiKey.mockReturnValue({
    mutateAsync: vi.fn().mockResolvedValue({ api_key_id: 'new-id', token: 'hrpv_1_test_token' }),
    isPending: false,
...
describe('ApiKeysPage', () => {
  ...
  it('shows revoke button only for non-revoked keys', () => {
    const active = makeKey({ id: 'key-1', name: 'active-key', revoked_at: null })
  ...
  it('calls revoke mutation when Révoquer clicked and confirmed', async () => {
    ...
    await waitFor(() => {
      expect(mockUseRevokeApiKey('wallet-1').mutateAsync).toHaveBeenCalledWith('key-uuid-1')
    })
  })
  ...
  it('does not call revoke mutation when confirm is cancelled', async () => {
    ...
    await waitFor(() => {
      expect(mockUseRevokeApiKey('wallet-1').mutateAsync).not.toHaveBeenCalled()
    })
  })
...
describe('ApiKeyTokenModal', () => {
  ...
  it('shows the warning message', () => {
    renderModal('hrpv_1_token')
    expect(screen.getByText('This token will NEVER be shown again.')).toBeInTheDocument()
  })
