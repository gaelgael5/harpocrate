  ...
  function confirmDelete(node: ReplicationNode) {
    modals.openConfirmModal({
      ...
      children: (
        <Stack gap="xs">
          <Text size="sm">
            {t('admin.replication.streaming.deleteConfirmDesc', { label: node.label })}
          </Text>
  ...
  return (
    <Card withBorder>
      <Stack>
        ...
        {nodes.error && (
          <Alert color="red">{String(nodes.error)}</Alert>
        )}

        {!nodes.isLoading && !nodes.error && (nodes.data?.nodes.length ?? 0) === 0 && (
        ...
        {!nodes.isLoading && !nodes.error && (nodes.data?.nodes.length ?? 0) > 0 && (
          <Table highlightOnHover>
            <Table.Thead>
              <Table.Tr>
                <Table.Th>{t('admin.replication.streaming.colLabel')}</Table.Th>
                <Table.Th>{t('admin.replication.streaming.colHost')}</Table.Th>
                <Table.Th>{t('admin.replication.streaming.colRole')}</Table.Th>
                <Table.Th>{t('admin.replication.streaming.colState')}</Table.Th>
                <Table.Th>{t('admin.replication.streaming.colLag')}</Table.Th>
                <Table.Th>{t('admin.replication.streaming.colLastSeen')}</Table.Th>
                <Table.Th />
              </Table.Tr>
            </Table.Thead>
            <Table.Tbody>
...
function AddNodeModal({
  ...
  function handleSubmit(values: AddFormValues) {
    const payload: AddStreamingNodePayload = {
      ...
      port: typeof values.port === 'number' ? values.port : Number(values.port) || 5432,
      role: values.role,
      ...
      standby_data_dir: values.standby_data_dir.trim() || '/var/lib/postgresql/16/main',
    }
        ...
        <Stack gap="sm">
          ...
          <Group grow>
            <TextInput
              ...
              description={t('admin.replication.streaming.fieldStandbyHostHint')}
          ...
          <Select
            ...
            data={[
              { value: 'standby_ro', label: t('admin.replication.streaming.roleStandbyRo') },
              {
