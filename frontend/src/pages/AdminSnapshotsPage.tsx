...
function PolicyModal({
  ...
  const [hourly, setHourly] = useState<number | string>(
    policy.retention.hourly,
  );
  ...
  const [monthly, setMonthly] = useState<number | string>(
    policy.retention.monthly,
  );
  const [yearly, setYearly] = useState<number | string>(
    policy.retention.yearly,
  );
  const [pushRemote, setPushRemote] = useState(
    policy.push_remote_after_snapshot,
  );
  ...
  const mut = useMutation({
    ...
    onSuccess: () => {
      notifications.show({
        color: "green",
        message: t("snapshots.policySaved"),
      });
  ...
  return (
    ...
    >
      <Stack>
        ...
        <SimpleGrid cols={5}>
          <NumberInput
            label={t("snapshots.hourly")}
            value={hourly}
            onChange={setHourly}
            min={0}
          />
          <NumberInput
            label={t("snapshots.daily")}
            value={daily}
            onChange={setDaily}
            min={0}
          />
          <NumberInput
            label={t("snapshots.weekly")}
            value={weekly}
            onChange={setWeekly}
            min={0}
          />
          <NumberInput
            label={t("snapshots.monthly")}
            value={monthly}
            onChange={setMonthly}
            min={0}
          />
          <NumberInput
            label={t("snapshots.yearly")}
            value={yearly}
            onChange={setYearly}
            min={0}
          />
...
export function AdminSnapshotsPage() {
  ...
  const { data: history, isLoading: histLoading } = useQuery({
    ...
    queryFn: () =>
      fetchSnapshotHistory({ tier: tierFilter ?? undefined, limit: 100 }),
  });

  const triggerMut = useMutation({
    mutationFn: (force: boolean) =>
      triggerSnapshot({ force, description: "Manual snapshot" }),
    onSuccess: (result) => {
      if (result.skipped) {
        notifications.show({
          color: "yellow",
          message: t("snapshots.skippedNoChange"),
        });
      } else {
        notifications.show({
          color: "green",
          message: t("snapshots.triggerSuccess"),
        });
