...
import { SecretListResponseSchema, SecretDetailResponseSchema } from '@/schemas/secrets'
...
export function ExportAllPage() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      let secretsTotal = wallets.reduce((a, w) => a + w.valued_secrets_count, 0)
      let done = 0
      setProgress({ current: 0, total: secretsTotal, step: t('export.stepDecrypting') })
  ...
  return (
    <Stack>
      ...
      {state === 'done' && (
        <Alert color="green">
          {t('export.success', { count: rowCount })}
        </Alert>
      )}
