  ...
  return (
    <Stack>
      ...
      <Card withBorder>
        <Stack>
          <Group justify="space-between">
            <Title order={4}>{t('admin.replication.streaming.lagOverTime')}</Title>
            <SegmentedControl
              size="xs"
              data={WINDOW_OPTIONS as unknown as { label: string; value: string }[]}
              value={hours}
              onChange={setHours}
            />
          </Group>
  ...
  return (
    <Card withBorder>
      <Stack gap="xs">
        ...
        <Group gap="xl">
          <KV label={t('admin.replication.streaming.colHost')} value={`${node.host}:${node.port}`} mono />
          <KV
  ...
  return (
    <Stack gap={4}>
      <svg
        ...
        style={{ width: '100%', height: 240, background: '#fafafa', borderRadius: 4 }}
...
function RecentStateChangesCard({
  ...
  const { t } = useTranslation()
  const changes = useMemo(() => extractStateChanges(observations), [observations])

  return (
    <Card withBorder>
      <Stack gap="xs">
        <Title order={5}>{t('admin.replication.streaming.recentStateChanges')}</Title>
        {changes.length === 0 ? (
          ...
          <Table>
            <Table.Thead>
              <Table.Tr>
                ...
                <Table.Th>{t('admin.replication.streaming.changeFrom')}</Table.Th>
