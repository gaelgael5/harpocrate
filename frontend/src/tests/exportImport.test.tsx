...
import { WalletExportSchema, WalletImportRequestSchema } from '@/schemas/exportImport'
        ...
        <QueryClientProvider client={makeQueryClient()}>
          <MemoryRouter initialEntries={['/wallets/wallet-1']}>
            <Routes>
              ...
              <Route path="/wallets/:walletId/secrets/new" element={<div>new secret</div>} />
            </Routes>
          </MemoryRouter>
        </QueryClientProvider>
      </MantineProvider>
        ...
        <QueryClientProvider client={makeQueryClient()}>
          <MemoryRouter initialEntries={['/wallets/import']}>
            <Routes>
              ...
              <Route path="/wallets/:walletId" element={<div data-testid="wallet-detail">wallet detail</div>} />
...
describe('WalletExportSchema', () => {
  ...
  it('rejects secret name with invalid characters', () => {
    const bad = {
      ...VALID_EXPORT,
      secrets: [
        { name: 'INVALID NAME!', is_placeholder: true, tags: [], generation_version: 1 },
      ],
    }
    const result = WalletExportSchema.safeParse(bad)
    expect(result.success).toBe(false)
...
describe('WalletImportPage', () => {
  ...
  it('shows error on invalid export format (wrong format_version)', async () => {
    ...
    const file = makeJsonFile({ format_version: '2', wallet: { name: 'test', tags: [] }, secrets: [] })
    const input = document.querySelector('input[type="file"]') as HTMLInputElement
    fireEvent.change(input, { target: { files: [file] } })
