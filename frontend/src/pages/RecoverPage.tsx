...
export function RecoverPage() {
  ...
  const [decryptedSymKey, setDecryptedSymKey] = useState<Uint8Array | null>(
    null,
  );
  const [decryptedRsaPriv, setDecryptedRsaPriv] = useState<Uint8Array | null>(
    null,
  );
      ...
      >
        <Stack gap="sm">
          <Alert
            color="red"
            variant="light"
            title={t("recover.destroyWarningTitle")}
          >
    ...
    async function load() {
      try {
        ...
        if (r.status === 410) {
          ...
          setErrorMsg(
            t(`recover.errors.${reason}`, t("recover.errors.session_invalid")),
          );
          setStep("error");
          return;
        }
  ...
  async function handleVerifyWords() {
    ...
    try {
      ...
      try {
        ...
        const encRsaPrivByRec = fromBase64(
          blobs.encrypted_rsa_private_key_by_recovery,
        );
