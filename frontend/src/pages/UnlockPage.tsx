...
export function UnlockPage() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        if (!authenticated) { navigate('/login', { replace: true }); return }
        setPageState('ready')
      } catch {
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.isFirstLogin) { navigate('/first-login', { replace: true }); return }
        if (err.isUnauthorized) { navigate('/login', { replace: true }); return }
        msg = err.message
      }
      setError(msg)
      setPageState('ready')
  ...
  if (pageState === 'check-auth') {
    return <Center h="100vh"><Loader size="lg" color="brand" /></Center>
  }
