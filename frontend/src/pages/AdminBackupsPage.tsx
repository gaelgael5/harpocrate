...
function RestoreModal({
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      notifications.show({
        color: "green",
        message: t("admin.backups.restoreSuccess", {
          epoch: result.session_epoch_new,
        }),
      });
    ...
    mutationFn: () =>
      enableMaintenance({
        ...
        estimated_duration_minutes:
          typeof duration === "number" ? duration : 30,
      }),
    onSuccess: () =>
...
export function AdminBackupsPage() {
  ...
  const pushRemoteMut = useMutation({
    ...
    onError: (err: unknown) => {
      const msg =
        err instanceof Error ? err.message : t("admin.backups.pushRemoteError");
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      notifications.show({
        color: "green",
        message: t("admin.backups.deleteSuccess"),
      });
      void qc.invalidateQueries({ queryKey: ["admin-backups"] });
    },
  });
                    ...
                    <Menu position="bottom-end" withinPortal>
                      <Menu.Target>
                        <Button
                          ...
                          color="brand"
                          loading={
                            pushRemoteMut.isPending &&
                            pushRemoteMut.variables?.backupId === b.id
                          }
                      ...
                      <Menu.Dropdown>
                        <Menu.Label>
                          {t("admin.backups.pushRemotePick")}
                        </Menu.Label>
                        {remotes.map((r) => (
                          <Menu.Item
                            key={r.id}
                            onClick={() =>
                              pushRemoteMut.mutate({
                                backupId: b.id,
                                remoteId: r.id,
                              })
                            }
