...
export function AdminSecretTypeDetailPage() {
  ...
  const [selectedVersion, setSelectedVersion] =
    useState<SchemaVersionFullLocal | null>(null);
      ...
      if (error.status === 404) {
        return (
          <Stack>
            <Alert
              color="orange"
              title={t("secret_types.notFound") || "Type introuvable"}
            >
              Le type {typeUuid} n'existe pas (ou plus). Il a peut-être été
              supprimé.
            </Alert>
            <Button
              variant="subtle"
  ...
  return (
    <Stack>
            ...
            {data.all_versions.map((v) => (
              ...
              >
                <Table.Td>
                  <Group gap="xs">
                    <Badge color="brand">
                      {t("secret_types.versionN", { n: v.version })}
                    </Badge>
                    {isCurrentVersion(v as SchemaVersionFullLocal) && (
                      <Badge color="green" size="xs">
                        {t("secret_types.currentBadge")}
                      </Badge>
                    )}
                  </Group>
                </Table.Td>
      ...
      <Modal
        ...
        title={
          selectedVersion
            ? t("secret_types.viewVersion", { n: selectedVersion.version })
            : ""
        }
        size="xl"
      >
      ...
      {newVersionOpen && (
        <NewVersionModal
          typeUuid={typeUuid!}
          previousVersion={
            data.all_versions[0] as SchemaVersionFullLocal | undefined
          }
        ...
        <Stack>
          ...
          <Group justify="flex-end">
            ...
            <Button
              ...
              onClick={() =>
                deleteVersionTarget &&
                deleteVersionMut.mutate(deleteVersionTarget.version_uuid)
              }
            >
              {t("common.delete")}
            </Button>
...
function NewVersionModal({
  ...
  const [schemaUi, setSchemaUi] = useState(
    previousVersion ? JSON.stringify(previousVersion.schema_ui, null, 2) : "{}",
  );
  ...
  const addMut = useMutation({
    ...
    onSuccess: () => {
      notifications.show({
        color: "green",
        message: t("secret_types.addVersionSuccess"),
      });
      onSuccess();
    },
    onError: (err: unknown) => {
  ...
  return (
    ...
    >
      <Stack>
        {previousVersion && (
          <Text size="xs" c="dimmed">
            {t("secret_types.prefillFromPrevious", {
              n: previousVersion.version,
            })}
          </Text>
        )}
