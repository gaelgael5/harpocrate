...
function MarkdownDoc({ filename }: { filename: string }) {
  ...
  if (error) {
    return (
      <Alert
        color="orange"
        title={t("integration.docNotAvailable") || "Documentation indisponible"}
      >
    ...
    return (
      <Card withBorder p="lg" radius="md">
        <Stack gap="md">
          ...
          <Text c="dimmed">
            {t("integration.plannedDesc", { sdk: sdk.name })}
          </Text>
        </Stack>
      </Card>
    );
            ...
            <Group gap="xs">
              {latestByKind.map((a) => (
                <Button
                  ...
                  title={
                    !a.available ? t("integration.artifactMissing") : a.filename
                  }
                >
                  {labelForKind(a.kind)}
                </Button>
...
export function IntegrationPage() {
  ...
  return (
    <Box
      style={{ background: "var(--mantine-color-body)", minHeight: "100vh" }}
    >
      ...
      <Container size="lg" py="xl">
        <Stack gap="xl" maw={860}>
          ...
          <Alert
            color="blue"
            variant="light"
            title={t("integration.apiKeyNote.title")}
          >
